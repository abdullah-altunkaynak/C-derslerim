...
typdef struct
{
	...
	1-<stdio.h>
		...
		*getchar(void) -> stdin (varsayýlan klavye) üzerinden tampon kullanmadan direkt olarak veri alýnmasýný saðlar. Parametre almaz geriye stdin den gelen verinin sayýsal kodunu
		dönderir.
		---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
		*printf("", ...) -> Ýlk parametresine yazýlan metin bazý kontrol karakterleri, sabitler ve deðiþkenler hariç direkt olarak yazdýrýlýr. Kontrol karakterlerinden % ve \ iþareti
		...
		---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
		*scanf("", ...) -> Yine içerisine belirsiz sayýda deðiþken alýr. Ýlk parametrede belirtildiði formatta stdin üzerinden (burada tampon console ekranýdýr.) istenile veri
...
ÖDEVLER
	...
	**-Mantýksal bir iliþki kurarak katarlar üzerinde iþlem yapan fonksiynolar ile ayrý ayrý örnekler yazýnýz.
	...
	-E-Ticaret sitesi kurduðunuzu ve üye ol, üye giriþi ve parolamý unuttum kýsmý olsun. Üye ol kýsmýnda da özel soru baþlýðý altýnda özel bir veri isteyin. Daha sonra parolamý unuttum
	yapýldýðý zaman o özel karakter ile sýfýrlama yapýlabilsin. Üye giriþi yapýldýktan sonra stok ta olan ürünler listelensin stokta biten ürünler görüntülenmesin fakat silinmesinde.
	Bir cüzdan olsun ve cüzdana giriþ yapýldýðýnda bakiye gösterilsin ve bakiye yükleme seçeneði olsun. Bakiye yüklemede 16 haneli bir kart numarasý istensin ve bu kart numarasý eðer
	birinci ve ikinci hanelerinin toplamýnýn birler basamaðý onaltýncý hanesinin basamaðýna eþit ve 2 ye bölünebilir olmasý þartý olsun. Bütün bunlar dosyalama iþlemi ile yapýlacaktýr.
...
void main()
{
	...
	fwrite("Abdullah ile c dersleri ikinci yazý !", 1/*Her verinin kaç byte olduðu bizimde char olduðu için 1*/, 50/*Kaç binary belirtildi ise tek eleman 1 olmak üzere *binary*/, f);
